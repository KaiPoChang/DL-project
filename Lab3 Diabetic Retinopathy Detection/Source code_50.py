...
def conv1x1(inplanes, outplanes, stride=1):
    return nn.Conv2d(inplanes, outplanes, kernel_size=1, stride=1, bias=False)

def conv3x3(inplanes, outplanes, stride=1, groups=1):
    return nn.Conv2d(inplanes, outplanes, kernel_size=3, stride=2, padding=1, groups=groups, bias=False)
...
optimizer = torch.optim.SGD(pretrain_resnet50.parameters(), lr=learning_rate, momentum = 0.9, weight_decay = 5e-4)
...
def train_model(model):
    ...
    if model == pretrain_resnet50:
        torch.save(model, 'pretrain_resnet50.pth.tar')
    elif model == unpretrain_resnet50:
        torch.save(model, 'unpretrain_resnet50.pth.tar')
...
unpretrain_resnet50 = ResNet(Bottleneck, [3, 4, 6, 3]).cuda()


# In[15]:


pretrain_resnet50 = pretrain_resnet50.cuda()
optimizer = torch.optim.SGD(pretrain_resnet50.parameters(), lr=learning_rate, momentum = 0.9, weight_decay = 5e-4)
pretrain_resnet50 = train_model(pretrain_resnet50)


# In[16]:


unpretrain_resnet50 = unpretrain_resnet50.cuda()
optimizer = torch.optim.SGD(unpretrain_resnet50.parameters(), lr=learning_rate, momentum = 0.9, weight_decay = 5e-4)
unpretrain_resnet50 = train_model(unpretrain_resnet50)
...
pre50_acc_train, pre50_acc_test, pre50_truth, pre50_predict = pretrain_resnet50
unpre50_acc_train, unpre50_acc_test, unpre50_truth, unpre50_predict = unpretrain_resnet50


# In[19]:


pre50_true = [i.data.cpu().numpy() for i in pre50_truth]
pre50_pred= [i.data.cpu().numpy() for i in pre50_predict]
unpre50_true = [i.data.cpu().numpy() for i in unpre50_truth]
unpre50_pred= [i.data.cpu().numpy() for i in unpre50_predict]


# In[20]:


plot_confusion_matrix(pre50_true, pre50_pred, classes=5, normalize=True)
plt.title('Normalized confusion matrix (Pretrained ResNet50)')
...
plot_confusion_matrix(unpre50_true, unpre50_pred, classes=5, normalize=True)
plt.title('Normalized confusion matrix (Unpretrained ResNet50)')
plt.xlabel('True label')
plt.ylabel('Predicted label')
plt.show()
...
plt.figure()
plt.plot(epoch, pre50_acc_train, label='Test (w/o pretraining)', color='blue')
plt.plot(epoch, pre50_acc_test, label='Test (with pretraining)', color='red')
plt.plot(epoch, unpre50_acc_train, label='Train (w/o pretraining)', color='green')
plt.plot(epoch, unpre50_acc_test, label='Train (with pretraining)', color='magenta')

plt.title('Result_comprehension (ResNet50)')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
